	...
	bool include_feature)
{
	...
	if (inside)
	{
		...
		dL_ddepth = dL_dpixels_depths[pix_id];
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			accum_rec_depths = last_alpha * last_depth + (1.f - last_alpha) * accum_rec_depths;
			const float c_d = collected_depths[j];
			last_depth = c_d;
			dL_dalpha += (c_d - accum_rec_depths) * dL_ddepth;
			atomicAdd(&(dL_ddepths[global_id]), dchannel_dcolor * dL_ddepth);
