	...
	const bool include_feature)
{
  ...
  torch::Tensor out_depths = torch::full({ H, W }, 0.0, float_opts);
  if (include_feature) {
  ...
  return std::make_tuple(rendered, out_color, out_depths, out_language_feature, radii, geomBuffer, binningBuffer, imgBuffer);
}
