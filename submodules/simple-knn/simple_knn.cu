...
__global__ void boxNearestIds(uint32_t P, float3* points, uint32_t* indices, MinMax* boxes, int* out_ids, int N)
{
	int halfN = N >> 1;
	int idx = cg::this_grid().thread_rank();
	if (idx >= P)
	...
	float3 point = points[indices[idx]];
	int point_id = indices[idx];
	int n = 0;

	for (int i = max(0, idx - halfN); i <= min(P - 1, idx + halfN); i++)
	{
		if (i == idx)
			continue;
		out_ids[point_id * N + n] = indices[i];
		n += 1;
	}
}
...
void SimpleKNN::getNearestIds(int P, float3* points, int* nearestIds, int N)
{
	float3* result;
	cudaMalloc(&result, sizeof(float3));
	size_t temp_storage_bytes;
	...
	thrust::device_vector<MinMax> boxes(num_boxes);
	boxMinMax << <num_boxes, BOX_SIZE >> > (P, points, indices_sorted.data().get(), boxes.data().get());
	boxNearestIds << <num_boxes, BOX_SIZE >> > (P, points, indices_sorted.data().get(), boxes.data().get(), nearestIds, N);
