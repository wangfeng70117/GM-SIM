...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        if training_args.include_feature:
            if self._language_feature is None or self._language_feature.shape[0] != self._xyz.shape[0]:
                print(f'Initializing language feature with shape {self._xyz.shape[0]}')
    ...
    def construct_features_list_of_attributes(self):
        l = ['x', 'y', 'z', 'nx', 'ny', 'nz']
        # All channels except the 3 DC
        for i in range(self._features_dc.shape[1] * self._features_dc.shape[2]):
            l.append('f_dc_{}'.format(i))
    ...
    def save_features_ply(self, path):
        mkdir_p(os.path.dirname(path))
        ...
        scale = self._scaling.detach().cpu().numpy()
        rotation = self._rotation.detach().cpu().numpy()
        language_features = self._language_feature.detach().cpu().numpy()

        dtype_full = [(attribute, 'f4') for attribute in self.construct_features_list_of_attributes()]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, language_features), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
    ...
    def load_features_ply(self, path):
        plydata = PlyData.read(path)
        ...
        lang_features = np.stack((np.asarray(plydata.elements[0]["lang_0"]),
                                  np.asarray(plydata.elements[0]["lang_1"]),
                                  np.asarray(plydata.elements[0]["lang_2"])), axis=1)
        ...
        self._language_feature = torch.tensor(lang_features, dtype=torch.float, device="cuda")
        self.active_sh_degree = self.max_sh_degree
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        if max_screen_size:
            ...
            # big_points_as = (self.get_scaling.max(dim=1).values / self.get_scaling.min(dim=1).values) > 10
            prune_mask = torch.logical_or(torch.logical_or(prune_mask, big_points_vs), big_points_ws)
            # prune_mask = torch.logical_or(prune_mask, big_points_as)
    ...
    def delete_points(self, mask):
        xyz_sub = self._xyz[mask].detach()
        features_dc_sub = self._features_dc[mask].detach()
        features_rest_sub = self._features_rest[mask].detach()
        opacity_sub = self._opacity[mask].detach()
        scaling_sub = self._scaling[mask].detach()
        rotation_sub = self._rotation[mask].detach()
        lang_features_sub = self._language_feature[mask].detach()
        ...
        self._language_feature = nn.Parameter(set_requires_grad(lang_features_sub, False))

    def reset_position(self, points):
        def set_requires_grad(tensor, requires_grad):
            """Returns a new tensor with the specified requires_grad setting."""
            return tensor.clone().requires_grad_(requires_grad)
