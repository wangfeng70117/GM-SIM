...
def densify_grids(
    ...
    for pi in range(init_particles.shape[0]):
        ...
        # 协方差的逆矩阵  公式11中计算密度场需要用到。
        cov = Q @ sig_mat @ Q.transpose()
        r = 0.0
        # 计算粒子的影响半径,特征值的平方根表示椭球的半径,而不是特征值
...
def fill_dense_grids(
    ...
    print(f'max_particles_per_cell is {max_particles_per_cell}')
    new_start_idx = start_idx
    for i, j, k in grid_density:
        if grid_density[i, j, k] > 0.0:
...
def fill_particles(
    ...
    if boundary is not None:
        ...
        for i in range(3):
            ...
            # x在0.4到1.6之间， y在0.4到1.6之间， z在0.4到1.6之间
    ...
    # fill internal grids
    # 填充内部网格部分
    print(f'search_exclude_dir: {search_exclude_dir}, ray_cast_dir: {ray_cast_dir}')
    # 这里是根据网格作为起点，作为射线的起点。
...
def init_filled_particles(pos, shs, cov, opacity, new_pos):
    ...
    # 根据位置赋值,找到每个新创建粒子距离最近的  GS重建出的原始粒子的位置,新创建粒子的属性值等于最近粒子的属性值
